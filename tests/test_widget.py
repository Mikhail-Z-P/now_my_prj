...
def test_single_digit(date_single_digit):
    input_val, expected = date_single_digit
    assert get_date(input_val) == expected
...
def test_empty(empty_str):
    with pytest.raises(ValueError):
        get_date(empty_str)

def test_whitespace(whitespace_str):
    with pytest.raises(ValueError):
        get_date(whitespace_str)